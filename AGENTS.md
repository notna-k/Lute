...
1. Copy `.env.example` to **`.env` at the repo root** (the Makefile passes `--env-file $(CURDIR)/.env` to compose) and set `JWT_SECRET`, `ADMIN_EMAIL`, and `ADMIN_PASSWORD` — the only three without defaults. Keep `ADMIN_PORT` and `API_HTTP_PORT` distinct; they are separate containers and the second to bind a shared port fails to start.
